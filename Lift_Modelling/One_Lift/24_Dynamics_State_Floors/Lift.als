...
fact first {
	...
	// ( all f: s.floors | f in Second => f.down= Not_Pressed and f not in s.lift.pressed_buttons)
}
...
fact down {
	...
	all s: State - last, s1: s.next {
		...
		// CHECK - if want to remove pressed_buttons constrained or not
		( s.lift.floor.value > s1.lift.floor.value ) =>
		{
		...
		(
			...
			(
				...
				(
					...
					( s.lift.floor.down = Pressed => s1.lift.dirc = Down ) and s1.lift.status = Moving 
				)) and
...
run{} for exactly 6 State, exactly 6 Lift,  21 Floor
