...
fact first {
	...
	some s: State - last, s1: s.next | s.lift.floor.door = Open and s1.lift.floor.door = Close and s1.lift.floor in Zero
	some s: State, f: s.floors | f in Second and f.down = Pressed and s.lift.floor
}
...
// written keeping Basic Sweep Algorithm in mind
pred lift_dirc_status ( s: State, s1: State )  {
	( s.lift.dirc = Up ) => 
	{
		...
		(
			( some f: s.floors | f.value > s.lift.floor.value and ( f in s.lift.pressed_buttons or f.up = Pressed or f.down = Pressed )) =>
		...
		(
			(( some f: s.floors | f.value < s.lift.floor.value and ( f in s.lift.pressed_buttons or f.up = Pressed or f.down = Pressed )) and 
			( no f: s.floors | f.value > s.lift.floor.value and ( f in s.lift.pressed_buttons or f.up = Pressed or f.down = Pressed ))) =>
			( s1.lift.dirc = Down and s1.lift.status = Moving )
		)

		// tackle the scenario in which no states have the need to be visited
		(
			(( no f: s.floors | f.value > s.lift.floor.value and ( f in s.lift.pressed_buttons or f.up = Pressed or f.down = Pressed )) and 
			( no f: s.floors | f.value < s.lift.floor.value and ( f in s.lift.pressed_buttons or f.up = Pressed or f.down = Pressed ))) =>
